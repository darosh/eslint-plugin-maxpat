...
export default function formatter (results = []) {
  ...
  for (const r of results) {
    ...
    files[r.filePath].push(...r.messages.filter(m => m.ruleId === 'maxpat/ui/annotation-info' && m.messageId === 'uiElementInfoJson').map(m => JSON.parse(m.message)))
  }
...
function printTables (data) {
  ...
  const proportions = [0.15, 0.15, 0.15, 0.15, 0.2, 0.2]
  ...
  for (const [file, entries] of Object.entries(data)) {
    ...
    const table = new Table({
      head: ['Type', 'Varname', 'Short', 'Long', 'Annotation Name', 'Annotation'].map((h) =>
        chalk.gray(h)
      ),
      style: {
