// lib/rules/check-disconnected-objects.js
export default {
  meta: {
    type: 'suggestion',
    docs: {
    ...
    return {
      ...
      JSONProperty (node) {
        ...
        boxesProperty.value.elements.forEach(boxNode => {
          ...
          if (skipObjects.includes(maxclass)) {
            return
          }

          // Skip inlet/outlet objects (they're part of abstraction interface)
          if (maxclass === 'inlet' || maxclass === 'outlet') {
            return
          }

          // Skip loadbang (often stands alone)
          if (maxclass === 'loadbang') {
            return
          }
