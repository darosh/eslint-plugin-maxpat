export default {
    ...
    schema: [{
      ...
      properties: {
        milliseconds: {
          ...
          minimum: 1,
          default: 1,
          description: 'Suggested milliseconds'
        },
        include: {
          type: 'array',
          items: { type: 'string' },
  ...
  create (context) {
    const [{ milliseconds = 1, include = [] }] = context.options
    ...
    function traversePatcher (patcherNode) {
      ...
      boxesProperty.value.elements.forEach(boxNode => {
        ...
        if (!include.includes(objectType)) {
          return
        }
        ...
        if (currentSpeedLimit !== milliseconds) {
          const messageData = {
            objectType: maxclass,
          ...
          context.report({
            ...
            fix: context.options[0]?.fix === true ? (fixer) => {
              ...
              if (hasSpeedLimitProperty) {
                ...
                return fixer.replaceText(speedLimProperty.value, milliseconds.toString())
              } else {
                ...
                if (savedAttrProperty && savedAttrProperty.value.type === 'JSONObjectExpression') {
                  ...
                  if (savedAttrProps.length > 0) {
                    ...
                    const insertText = `,\n\t\t\t\t"parameter_speedlim" : ${milliseconds}`
