export function getOverlapsCreate (targetRect = 'patching_rect', presentation = 0) {
  return (context) => {
    ...
    function getObjectRect (boxProps) {
      const patchrectProperty = boxProps.find(p => p.key.value === targetRect)
    ...
    function traversePatcher (patcherNode) {
      ...
      boxesProperty.value.elements.forEach(boxNode => {
        ...
        if (presentation && (boxProps.find(p => p.key.value === 'presentation')?.value?.value !== presentation)) {
          return
        }
      ...
      for (let i = 0; i < objects.length; i++) {
        for (let j = i + 1; j < objects.length; j++) {
          ...
          if (overlapArea > threshold) {
            ...
            const data = {
              mode: presentation ? 'presentation' : 'patching',
