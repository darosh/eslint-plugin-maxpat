                  ...
                  if (newPositions.has(objId)) {
                    ...
                    if (rectProp?.value?.elements?.length === 4) {
                      ...
                      if (xNode && yNode && wNode && hNode &&
                        ...
                        // Round values to prevent conflicts with coordinate-rounding rule
                        const x = parseFloat(newLayout.position[0].toFixed(1))
                        const y = parseFloat(newLayout.position[1].toFixed(1))
                        const w = parseFloat(newLayout.size[0].toFixed(1))
                        const h = parseFloat(newLayout.size[1].toFixed(1))

                        fixes.push(fixer.replaceText(xNode, x.toString()))
                        fixes.push(fixer.replaceText(yNode, y.toString()))
                        fixes.push(fixer.replaceText(wNode, w.toString()))
                        fixes.push(fixer.replaceText(hNode, h.toString()))
                      }
                    }
                  }
                }
