const defaultOptions = [{ deprecated: ['gate~', 'switch~', 'scope~', 'poly~', 'drunk'] }]
...
export default {
  meta: {
    ...
    schema: [ // Defines the rule's options
      {
        ...
        properties: {
          deprecated: {
            ...
            items: { type: 'string' },
            uniqueItems: true,
            default: defaultOptions[0].deprecated,
            description: 'Lists deprecated objects'
          }
        },
    ...
    messages:
      {
        missingTextProp: `Missing text property.`,
        deprecatedObj:
          `The object "{{maxclassName}}" is deprecated. Consider alternatives.`
      }
  }
  ,
  create (context) {
    const deprecatedObjects = context.options[0]?.deprecated || [] // Get the list from config
